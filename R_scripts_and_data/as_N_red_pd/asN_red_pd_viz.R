...
library(vegan)
library(tidyverse)
setwd("/Users/luiszuniga/Desktop/faiths2/as_N_red")

# importing dissimilatory nitrate reduction data

asN_red_table <- read.csv(file = "asN_red_pd.csv", row.names = 1)
asN_red_table <- as.matrix(asN_red_table)

# shapiro test for normality
shapiro.test(asN_red_table)
...
# CCA oordination
cca<-cca(asN_red_table)
# NMDS oordination
nmds<-metaMDS(asN_red_table, distance = "bray")
# stress plot of nmds
stressplot(nmds)
nmds$stress
# plot the NMDS
nmds.sp.fit <- envfit(nmds, asN_red_table)
plot(nmds, type = "n")
plot(nmds.sp.fit)
